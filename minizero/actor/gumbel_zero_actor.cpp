...
std::string GumbelZeroActor::getActionComment()
{
...
void GumbelZeroActor::beforeNNEvaluation()
{
    ...
    if (mcts_.getNumSimulation() == 0) {
        ...
        node_path = mcts_.selectFromNode(candidates_[0]);
        node_path.insert(node_path.begin(), mcts_.getRootNode());
    }
