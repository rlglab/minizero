...
void setConfiguration(ConfigureLoader& cl)
{
    ...
    cl.addParameter("actor_mcts_value_rescale", actor_mcts_value_rescale, "true for games whose rewards are not bounded in [-1, 1]", "Actor");             // ref: MZ
    ...
    cl.addParameter("zero_actor_intermediate_sequence_length", zero_actor_intermediate_sequence_length, "the max sequence length when running self-play; usually 0 (unlimited) for board games", "Zero"); // ref: MZ
    ...
    cl.addParameter("learner_n_step_return", learner_n_step_return, "the number of steps to calculate the n-step value; usually 0 for board games", "Learner"); // ref: MZ
