...
The performance of each zero-knowledge learning algorithm on Go and Othello are shown as follows, where α<sub>0</sub>, μ<sub>0</sub>, g-α<sub>0</sub>, and g-μ<sub>0</sub> represent AlphaZero, MuZero, Gumbel AlphaZero, and Gumbel MuZero, and $n$ represents simulation count.
...
To facilitate the use of MiniZero, a `docker/Dockerfile` is provided to build the required environment.
A container tool such as `docker` or `podman` is required.
